...
def val_model(dloader, model, processor, image_id_dict, config):
    ...
    with torch.no_grad():
        for idx, (q_texts, encodings, imgs, labels, img_paths) in progress_bar(
            enumerate(dloader), total=len(dloader)
        ):
            outputs = model.generate(encodings, imgs)
            text_outputs = [processor.decode(output, skip_special_tokens=True) for output in outputs]

            if idx % 50 == 0:
                print(f"[INFO] Batch {idx}: Sample question {q_texts[0]}")
                print(f"[INFO] -> {text_outputs[0]}")
    ...
    # ✅ Write predictions
    out_path = os.path.join('multi_frame_results', config.model_name, 'finalpredictions.json')
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with open(out_path, 'w') as f:
        json.dump(test_data, f)
...
def save_experiment(coco_eval, config):
    ...
    trial_data.update(coco_eval.eval)
...
def params():
    ...
    parser.add_argument("--annotation-file", default=os.path.join('data','multi_frame','multi_frame_test_coco.json'),
                        type=str, help="COCO-style GT file for caption eval")
    # NEW CLI args
    parser.add_argument("--input-json", default="data/multi_frame/finaldummyprompts.json",
                        type=str, help="Path to prompts JSON")
    parser.add_argument("--image-id-json", default="data/multi_frame/final_image_id_dummy.json",
                        type=str, help="Path to image_id mapping JSON")
    return parser.parse_args()
...
if __name__ == "__main__":
    ...
    with open(config.image_id_json) as f:
    ...
    # COCO eval
    annotation_file = config.annotation_file
    results_file = os.path.join('multi_frame_results', config.model_name, 'finalpredictions.json')
    coco = COCO(annotation_file)
    coco_result = coco.loadRes(results_file)
    coco_eval = COCOEvalCap(coco, coco_result)
    coco_eval.params['image_id'] = coco_result.getImgIds()
